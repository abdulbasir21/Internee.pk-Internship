...
export default function DashboardShell({ roleLabel, icon, name, accentGradient, onLogout, children }) {
    ...
    <>
      ...
      <div className="relative px-7 pt-8 pb-6">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl flex items-center justify-center shrink-0 shadow-lg" style={{ background: gradient }}>
      ...
      <div className="relative px-5 py-6">
        <div className="rounded-2xl px-4 py-4 flex items-center gap-3"
          ...
          <div className="w-11 h-11 rounded-full flex items-center justify-center text-white text-sm font-bold shrink-0 shadow-md" style={{ background: gradient }}>
            {icon}
          </div>
          ...
          <div className="ml-auto w-2 h-2 rounded-full shrink-0" style={{ background: "#22C55E", boxShadow: "0 0 6px #22C55E" }} />
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row" style={{ background: "#0F0E17" }}>

      {/* ── Mobile top bar ── */}
      <div className="md:hidden flex items-center justify-between px-5 py-4"
        style={{ background: "#1A1828", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-xl flex items-center justify-center" style={{ background: gradient }}>
            <Sparkles size={14} className="text-white" />
          </div>
          <span className="text-white text-sm font-semibold">Intern Tracker</span>
        </div>
      ...
      {sidebarOpen && (
        <div className="md:hidden flex flex-col relative overflow-hidden"
          style={{ background: "linear-gradient(180deg, #1A1828 0%, #13121F 100%)", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
          <SidebarContent />
        </div>
      )}

      {/* ── Desktop sidebar ── */}
      <aside className="hidden md:flex w-72 flex-col shrink-0 relative overflow-hidden"
        style={{ background: "linear-gradient(180deg, #1A1828 0%, #13121F 100%)", borderRight: "1px solid rgba(255,255,255,0.06)" }}>
        <SidebarContent />
      </aside>

      {/* ── Main content ── */}
      <main className="flex-1 overflow-auto" style={{ background: "#F7F6FB" }}>
